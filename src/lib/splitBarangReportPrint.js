...
export const printSplitBarangReport = (data, startDate, endDate) => {
  ...
  const rowsHtml = (data || []).map((parentItem) => {
    ...
    const kodeBarang = parentItem.kode_item_barang || '-';
    ...
    if (children.length > 0) {
      anakRows = children.map((anak) => {
        ...
        return `
          <tr class="anak-row">
            <td align="center">↳</td>
            <td align="left">${anak.kode_item_barang || '-'}</td>
            <td align="left">${anak.nama_item_barang || '-'}</td>
            <td align="left">${anak.dimensi_string || '-'}</td>
            <td align="right">${anak.qty_item ?? 0}</td>
    ...
    return `
      <tr class="parent-row">
        ...
        <td align="left" style="font-weight: bold;">${kodeBarang}</td>
        <td align="left" style="font-weight: bold;">${namaBarang}</td>
        <td align="left" style="font-weight: bold;">${parentItem.dimensi_string || '-'}</td>
        <td align="right" style="font-weight: bold;">${parentItem.qty_item ?? 0}</td>
  ...
  const documentHtml = `
    <!DOCTYPE html><html lang="id"><head>
      ...
      <style>
        ...
        td { padding:4px 4px; font-size:10px; border-bottom:1px solid #ddd; }
        .parent-row td { background:#e2e8f0; border-top: 1px solid #94a3b8; }
