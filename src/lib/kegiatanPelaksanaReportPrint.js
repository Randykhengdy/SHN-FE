import { format } from 'date-fns';

const formatDecimal2 = (n) => (n === undefined || n === null ? '0.00' : Number(n).toFixed(2));

/**
 * Print Report Kegiatan Pelaksana Kerja / Tanggal
 *
 * data[] = [{
 *   pelaksana, rows: [{ realisasi, tgl_wo, no_wo, nama_barang, status, qty, kilo, dimensi }]
 * }]
 *
 * Layout (sesuai screenshot):
 *   Pelaksana : AR , ARIF         ← group header
 *     06-03-2026 | 06-03-2026 | SO .260306.003 | ALUMINIUM PLAT... | Potong | 1 | 17.20 | 30X1250X2500
 *   SUB TOTAL -> ARIF     30    193.53
 *   GRAND TOTAL
 */
export const printKegiatanPelaksanaReport = (data, startDate, endDate) => {
  const periodeStart = startDate ? format(new Date(startDate), 'dd-MM-yyyy') : '-';
  const periodeEnd   = endDate   ? format(new Date(endDate),   'dd-MM-yyyy') : '-';
  const todayStr     = format(new Date(), 'EEEE, d MMMM yyyy');

  let gtQty = 0, gtKilo = 0;

  const pelaksanaBlocks = (data || []).map((grup) => {
    const rows = grup.rows || [];
    let stQty = 0, stKilo = 0;

    const rowHtml = rows.map((row) => {
      stQty  += Number(row.qty  ?? 0);
      stKilo += Number(row.kilo ?? 0);

      return `
        <tr>
          <td align="center">${row.realisasi    || ''}</td>
          <td align="center">${row.tgl_wo        || ''}</td>
          <td align="left"  >${row.no_wo          || ''}</td>
          <td align="left"  >${row.nama_barang   || ''}</td>
          <td align="center">${row.status         || ''}</td>
          <td align="right" >${row.qty            ?? 0}</td>
          <td align="right" >${formatDecimal2(row.kilo)}</td>
          <td align="left"  >${row.dimensi        || ''}</td>
        </tr>
      `;
    }).join('');

    gtQty  += stQty;
    gtKilo += stKilo;

    // Ambil nama singkat pelaksana untuk subtotal label
    const labelParts = (grup.pelaksana || '').split(',');
    const shortLabel = (labelParts[labelParts.length - 1] || grup.pelaksana || '').trim();

    return `
      <tr class="pelaksana-header">
        <td colspan="8" style="font-weight:bold; padding:7px 4px 3px 4px;">Pelaksana : ${grup.pelaksana || '-'}</td>
      </tr>
      ${rowHtml}
      <tr class="subtotal-row">
        <td colspan="5" align="right" style="font-weight:bold; border-top:1px solid #555; padding-right:8px; font-size:10px;">
          SUB TOTAL &rarr; ${shortLabel}
        </td>
        <td align="right" style="font-weight:bold; border-top:1px solid #555;">${stQty}</td>
        <td align="right" style="font-weight:bold; border-top:1px solid #555;">${formatDecimal2(stKilo)}</td>
        <td style="border-top:1px solid #555;"></td>
      </tr>
    `;
  }).join('');

  const documentHtml = `
    <!DOCTYPE html><html lang="id"><head>
      <meta charset="UTF-8">
      <title>Kegiatan Pelaksana / Tanggal</title>
      <style>
        body { font-family:'Courier New',Courier,monospace; font-size:10px; margin:0; padding:60px 20px 20px; background:#f5f5f5; }
        .report-page { background:white; padding:25px 35px; box-shadow:0 0 10px rgba(0,0,0,.1); min-height:21cm; margin:0 auto; width:36cm; }
        .toolbar { position:fixed; top:0; left:0; right:0; height:50px; background:#1e293b; color:white; display:flex; align-items:center; padding:0 20px; gap:10px; z-index:1000; }
        .toolbar-title { font-family:sans-serif; font-size:14px; font-weight:bold; margin-right:auto; }
        .btn { padding:6px 15px; border-radius:4px; border:none; cursor:pointer; font-weight:bold; font-family:sans-serif; font-size:12px; }
        .btn-print { background:#2563eb; color:white; } .btn-close { background:#ef4444; color:white; }
        .header { text-align:center; margin-bottom:14px; position:relative; }
        .header h1 { font-size:14px; margin:0 0 4px; text-decoration:underline; font-family:sans-serif; }
        .header .periode { font-size:11px; font-weight:bold; font-family:sans-serif; }
        .top-right-info { position:absolute; top:0; right:0; text-align:right; font-family:sans-serif; font-size:10px; }
        table { width:100%; border-collapse:collapse; margin-bottom:20px; }
        th { border-top:2px solid #000; border-bottom:2px solid #000; padding:4px 3px; text-align:center; font-weight:bold; font-size:10px; }
        td { padding:2px 3px; font-size:10px; }
        .pelaksana-header td { background:#f0f0f0; }
        .subtotal-row td { padding-top:3px; padding-bottom:6px; }
        .grand-total-row td { border-top:2px solid #000; border-bottom:2px double #000; font-weight:bold; padding:4px 3px; }
        @media print {
          .toolbar { display:none!important; }
          body { padding:0; background:white; }
          .report-page { padding:0; box-shadow:none; width:100%; }
          @page { size:A3 landscape; margin:1cm; }
        }
      </style>
    </head><body>
      <div class="toolbar">
        <span class="toolbar-title">Print Preview – Kegiatan Pelaksana / Tanggal</span>
        <button class="btn btn-print" onclick="window.print()">Print Sekarang</button>
        <button class="btn btn-close" onclick="window.close()">Tutup Preview</button>
      </div>
      <div class="report-page">
        <div class="header">
          <div class="top-right-info"><div>${todayStr}</div><div>Halaman : 001</div></div>
          <h1>Kegiatan Pelaksana / Tanggal</h1>
          <div class="periode">Periode : ${periodeStart} s.d. ${periodeEnd}</div>
        </div>
        <table>
          <thead>
            <tr>
              <th width="10%">Realisasi</th>
              <th width="10%">Tgl. WO</th>
              <th width="14%">No. WO</th>
              <th width="32%" align="left">Nama Barang</th>
              <th width="8%">Status</th>
              <th width="6%">Qty</th>
              <th width="8%">Kilo</th>
              <th width="12%" align="left">Dimensi</th>
            </tr>
          </thead>
          <tbody>
            ${pelaksanaBlocks || '<tr><td colspan="8" align="center" style="padding:20px;">Tidak ada data</td></tr>'}
            <tr class="grand-total-row">
              <td colspan="5" align="center">GRAND TOTAL</td>
              <td align="right">${gtQty}</td>
              <td align="right">${formatDecimal2(gtKilo)}</td>
              <td></td>
            </tr>
          </tbody>
        </table>
      </div>
    </body></html>`;

  const w = window.open('', '_blank');
  w.document.write(documentHtml);
  w.document.close();
};
