/**
 * Utility to generate and print Sales Order / Tanggal report
 */
...
export const printSalesOrderTanggalReport = (data, dateFrom, dateTo) => {
  ...
  const calculateGrandTotals = () => {
    return data.reduce((acc, item) => {
      acc.total += parseFloat(item.total_harga_so || 0);
      acc.discount += parseFloat(item.total_diskon || 0);
      ...
      return acc;
    }, { total: 0, discount: 0, biayaLain: 0, uangMuka: 0, sisaBayar: 0 });
  };
  ...
  let html = `
    ...
    <html>
    <head>
      <title>Sales Order / Tanggal</title>
      <style>
        body {
    ...
    <body>
      <div class="toolbar">
        <span class="toolbar-title">Print Preview - Sales Order / Tanggal</span>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th width="100" class="text-right">Sisa Bayar</th>
            <th width="120">Status SO</th>
          </tr>
        </thead>
        <tbody>
  `;
  ...
  sortedDates.forEach(date => {
    ...
    items.forEach((item, index) => {
      html += `
        <tr>
          <td>${index === 0 ? formatDate(item.tanggal_so) : ''}</td>
          <td>${item.nomor_so}</td>
          <td>${item.gudang?.nama_gudang || ''}</td>
          <td>${item.pelanggan?.nama_pelanggan || ''}</td>
          <td class="text-right">${formatCurrency(item.total_harga_so)}</td>
          <td class="text-right">${formatCurrency(item.total_diskon)}</td>
          <td class="text-right">${formatCurrency(item.biaya_lain)}</td>
          <td class="text-right">${formatCurrency(item.uang_muka)}</td>
          <td class="text-right">${formatCurrency(item.sisa_bayar)}</td>
