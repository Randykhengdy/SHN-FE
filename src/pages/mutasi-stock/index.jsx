...
const statusOptions = [
    ...
    { value: "partial-accepted", label: "Partial Accepted" },
];
...
export default function MutasiStockPage() {
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            ...
            const response = await stockMutationService.delete(selectedStockMutation.id);

            console.log('✅ Stock Mutation deleted:', response);

            // Close modal first
            setShowDeleteModal(false);

            // Show success message and reload data after alert closes
            showAlert("Sukses", "Mutation Stock berhasil dihapus!", "success", () => {
                loadStockMutation();
            });

        } catch (error) {
            console.error('❌ Error deleting Stock Mutation:', error);
            showAlert("Error", "Gagal menghapus Mutation Stock", "error");
        } finally {
            setIsDeleting(false);
        }
    };
    ...
    const handleView = (id) => {
        navigate(`/mutasi-stock/view/${id}`);
    };

    const handleDeleteStockMutation = (sm) => {
        setSelectedStockMutation(sm);
        setShowDeleteModal(true);
    };

    return <PageLayout title="Mutasi Stock" category="TRANSAKSI">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
            <div className="bg-white rounded-lg px-4 py-2 border border-gray-200">
                <h2 className="text-lg font-semibold text-gray-800">Daftar Mutasi Stock</h2>
            </div>
                                    ...
                                    <TableRow key={sm.id} className="hover:bg-gray-50">
                                        ...
                                        <TableCell>
                                            <div className="flex flex-col sm:flex-row gap-2 justify-center">
                                                <Button size="sm" variant="outline" onClick={() => handleView(sm.id)}>
                                                ...
                                                {isAdmin() && (
                                                    <Button
                                                        size="sm"
                                                        variant="destructive"
                                                        onClick={() => handleDeleteStockMutation(sm)}
                                                        title="Hapus Stock Mutation"
                                                    >
